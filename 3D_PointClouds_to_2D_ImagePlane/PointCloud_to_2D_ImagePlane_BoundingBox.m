...
if ~isempty(bounding_box)
    %% Por objeto no frame: Obtenha os dados de posição de cada objeto no frame
    places=bounding_box(:,4:6);
    size_=bounding_box(:,1:3);
    rotates=bounding_box(:,7);
    y_class=bounding_box(:,8:10);
    rotates = pi / 2 - rotates;
    %% Por frame independente da quantidade de objetos: calibração dos pontos velodyne com a imagem RGB
