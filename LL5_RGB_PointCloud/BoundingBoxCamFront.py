...
DATA_PATH = 'E:/LyfLevel5/v1.02-train/3d-object-detection-for-autonomous-vehicles/'
...
pointclouds_final=[]
boxes_cam_final2D=[]
#### Front Camera; Images from the front camera
for j in range(len(total_scene)):
    #print('scene:', j)
    ...
    for v in range(len(first_next_last_token)):
        ...
        for k in range(len(my_sample_gledson['anns'])):
            ...
            if len(data_path_cam)!=0 and len(camera)!=0 and len(corners_camera3D)!=0 and len(corners_camera2D)!=0 and len(data_path_lidar)!=0 and len(corners_lidar)!=0 and len(pointclouds)!=0 and len(categoria)!=0:
                sio.savemat('E:/LyfLevel5/DatasetLyft/Matlab/data_path_cam/'+str(j)+'_'+str(v)+'_'+str(k)+'.mat',{'data_path_cam':data_path_cam})
                sio.savemat('E:/LyfLevel5/DatasetLyft/Matlab/camera/'+str(j)+'_'+str(v)+'_'+str(k)+'.mat',{'camera':camera})
                sio.savemat('E:/LyfLevel5/DatasetLyft/Matlab/corners_camera3D/'+str(j)+'_'+str(v)+'_'+str(k)+'.mat',{'corners_camera3D':corners_camera3D})
                sio.savemat('E:/LyfLevel5/DatasetLyft/Matlab/corners_camera2D/'+str(j)+'_'+str(v)+'_'+str(k)+'.mat',{'corners_camera2D':corners_camera2D})
                sio.savemat('E:/LyfLevel5/DatasetLyft/Matlab/data_path_lidar/'+str(j)+'_'+str(v)+'_'+str(k)+'.mat',{'data_path_lidar':data_path_lidar})
                sio.savemat('E:/LyfLevel5/DatasetLyft/Matlab/corners_lidar/'+str(j)+'_'+str(v)+'_'+str(k)+'.mat',{'corners_lidar':corners_lidar})
                sio.savemat('E:/LyfLevel5/DatasetLyft/Matlab/pointclouds/'+str(j)+'_'+str(v)+'_'+str(k)+'.mat',{'pointclouds':pointclouds})
                sio.savemat('E:/LyfLevel5/DatasetLyft/Matlab/categoria/'+str(j)+'_'+str(v)+'_'+str(k)+'.mat',{'categoria':categoria})
